...
systemSelector.forEach(function (button) {
  button.addEventListener("click", function () {
    selectOS(button);
  });
});
document.getElementById("list-os-list").addEventListener("click", function () {
...
function detectOS() {
  ...
  if (navigator.appVersion.indexOf("Mac") != -1) osName = "MacOS", os = "mac"; //   if (navigator.appVersion.indexOf("X11") != -1)
  //     (osName = "UNIX"), (os = "unix");
  ...
  if (os != null) {
    document.getElementById("sysSelector").querySelectorAll("a").forEach(function (button) {
...
function selectOS(button) {
  ...
  var config = new bootstrap.Tab(document.querySelector('#configurator a[href="#list-config"]'));
  config.show();
}
...
function generate(mID, mPass) {
  ...
  var install = new bootstrap.Tab(document.querySelector('#configurator a[href="#list-install"]'));
  install.show();
}

function download() {
  var filename = "pleaseRenameMe.inetloc";
  var xmltext = "<?xml version='1.0' encoding='UTF-8'?>" + "<!DOCTYPE plist PUBLIC '-//Apple//DTD PLIST 1.0//EN' 'http://www.apple.com/DTDs/PropertyList-1.0.dtd'>" + "<plist version='1.0'><dict><key>URL</key><string>" + url + "</string></dict></plist>";
